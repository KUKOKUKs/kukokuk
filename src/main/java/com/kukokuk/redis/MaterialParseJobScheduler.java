...
public class MaterialParseJobScheduler {
    ...
    /**
     * 1. @Scheduled로 주기적으로 Redis 큐에서 leftPop() 2. ObjectMapper로 파싱 → jobNo, url 추출 3. Job 상태
     * IN_PROGRESS로 변경 4. Python 서버에 요청 보내고 응답받음 5. 응답 결과를 DB에 저장 (dailyStudyMaterial) 6. 성공 시
     * MaterialParseJob 상태 SUCCESS로 변경 7. 예외 시 MaterialParseJob 상태 FAILED로 변경 + 로그
     */

 ...
 */
async function updateOrCreateLog() {
    try {
        // SSR로 넘겨받은 이력정보가 없으면, 이력 생성요청
        if (!studyLog) {
            const data = await apiCreateStudyLog(dailyStudy.dailyStudyNo);
            console.log('학습 이력 생성 성공');

            studyLog = data; // 전역 변수 갱신
            studyLogNo = data.dailyStudyLogNo; // 전역 변수 갱신
        }
        // 학습이력이 이미 존재하면, 이력 수정요청
        else {
            const data = await apiUpdateStudyLog(studyLogNo);
            console.log('학습 이력 업데이트 성공');
    ...
    try {
        ...
        if (currentIndex < totalCount - 1) {
            ...
            if (currentIndex <= cardLastIndex && currentIndex > studyLog.studiedCardCount - 1) {
                ...
                // 학습 이력을 수정하는 비동기 요청 호출 (requestBody 전달)
                const data = await apiUpdateStudyLog(studyLogNo, {
                    studiedCardCount : updateStudiedCartCount
                });

                console.log('학습 이력 학습카드 개수 업데이트 성공');
                // js 전역객체 갱신
                studyLog.studiedCardCount = data.studiedCardCount;
            }
        }
        // 마지막 인덱스면 이력업데이트 및 학습끝내기
            ...
            const isQuestCompleted = data.questCompleted;
        ...
        console.error("학습 이력 업데이트 실패:", err);
        alert("학습 이력 업데이트 중 문제가 발생했습니다. 다시 시도해주세요.");
    }
});
...
/**
 * 퀴즈 렌더링 화면에서 option 버튼 클릭시 selected 되는 이벤트
 */
 ...
 */
$(document).on('click', '#quiz-submit-btn', async function () {
    ...
    try {
        ...
        if (studyQuizLogNo && studyQuizLogNo !== '') {
            // 퀴즈이력을 수정하는 비동기 요청 호출
            studyQuizLog = await apiUpdateQuizLog(studyQuizLogNo, selectedChoice);
            // quizLogs에서 해당 로그를 찾아 업데이트
            const indexToUpdate = quizLogs.findIndex(
                log => log.dailyStudyQuizLogNo === studyQuizLog.dailyStudyQuizLogNo);
            if (indexToUpdate !== -1) {
                quizLogs[indexToUpdate] = studyQuizLog;
            }
        }
        // 퀴즈 이력이 없으면 생성요청 및 quizLogs에 생성된 퀴즈 이력 추가
        else {
            // 퀴즈이력을 생성하는 비동기 요청 호출
            studyQuizLog = await apiCreateQuizLog(studyQuizNo, selectedChoice);

            // quizLogs에 넣어야 이 퀴즈화면이 다시 렌더링될 때 이력을 표시할 수 있음
            quizLogs.push(studyQuizLog);

            // 퀴즈컨테이너의 이력번호 속성도 변경해야 다시 요청보냈을 때 생성대신 수정 요청 전달 가능
            $quizContainer.attr('data-study-quiz-log-no', studyQuizLog.dailyStudyQuizLogNo);
        }

        // 사용자의 정답/오답 여부에 따라 selected된 보기에 스타일 추가
        markQuizResult(studyQuizLog.isSuccess);
        // 사용자의 정답/오답 여부에 따라 네비바 목록에 아이콘 추가
        updateQuizNavIcon(studyQuizNo, studyQuizLog.isSuccess);
    } catch (err) {
        alert("퀴즈 이력 저장에 실패했습니다. 다시 시도해주세요.");
    }
});

/**
 * isSuccess 여부로 현재 selected된 보기에 오답/정답 스타일을 추가하는 함수
 */
...
/**
 * 현재 인덱스(currentIndex)에 맞게 네비게이션(active 상태와 아이콘)을 렌더링하는 함수
 * - 카드 진행 시: 해당 card-index를 active 처리 + 체크 아이콘 추가
 * - 퀴즈 진행 시: 해당 quiz-index를 active 처리
 */
function renderNav() {
...
/**
 * 문자열 내 **마크다운 강조 문법**을 <strong> 태그로 변환하는 유틸 함수
 * @param text 원본 텍스트
...
/**
 * 객체/배열 전체를 순회하면서 applyStrong()을 적용하는 유틸 함수
 * @param content 변환할 데이터 구조
 * @returns {*|{}} 변환된 데이터 구조
 */
